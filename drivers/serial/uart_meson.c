 ...
 * NOTE: for some (unknown) reason only the current
 * number of RX/TX bytes in the FIFOs can be read.
 * All other FIFO related bits are zero.
 */
...
struct uart_meson_data {
	const struct device *clock;
	struct uart_config ucfg;
};

static void uart_meson_wait_tx_fifo_empty(const struct device *dev)
{
...
static void uart_meson_put_char(const struct device *dev, uint32_t c)
{
	const struct uart_device_config *config = dev->config;

	uart_meson_wait_tx_fifo_empty(dev);

	sys_write32(c, config->regs + AML_UART_WFIFO);
}

static int uart_meson_poll_in(const struct device *dev, unsigned char *c)
{
	const struct uart_device_config *config = dev->config;
	uint32_t status;
...
static void uart_meson_poll_out(const struct device *dev, unsigned char c)
{
	const struct uart_device_config *config = dev->config;
	uint32_t control;
...
static const struct uart_driver_api uart_meson_driver_api = {
	.poll_in	= uart_meson_poll_in,
	.poll_out	= uart_meson_poll_out,
	.err_check	= uart_meson_err_check,
};

static int uart_meson_init(const struct device *dev)
{
	const struct uart_device_config *config = dev->config;
	uint32_t control;
...
#define UART_MESON_INIT(index)						\
									\
static const struct uart_device_config uart_meson_cfg_##index = {	\
	.base = (uint8_t *)DT_INST_REG_ADDR(index),			\
};									\
									\
static struct uart_meson_data uart_meson_data_##index = {		\
	.ucfg = {							\
...
DEVICE_DT_INST_DEFINE(index,						\
		    ...
		    &uart_meson_data_##index,				\
		    &uart_meson_cfg_##index,				\
		    PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,	\
		    &uart_meson_driver_api);

DT_INST_FOREACH_STATUS_OKAY(UART_MESON_INIT)
